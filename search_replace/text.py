...
REPLACE_REGEX = '(?msi)^.*$'
REPLACE_HEADING = _('In field replacement mode, the specified field is set '
                'to the text and all previous values are erased. '
                'After replacement is finished, the text can be changed to '
                'upper-case, lower-case, or title-case.')

TEMPLATE_BUTTON_ToolTip = _('Open the template editor')

EXCEPTION_Invalid_identifier = _('Invalid identifier string. '
'It must be a comma-separated list of pairs of strings separated by a colon.')
...
def get_for_localized_field(field, value):
    return get_for_invalid_value(field, value)+'\n'+_('The value of this field is localized (translated). '
    'This can cause problems when using settings shared on internet or when changing the user interface language.')
